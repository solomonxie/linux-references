# $ venv/bin/python kern_05_wire_into_agent_tool.py
#
# Goal: compose steps 2-4 into one run_code_sandboxed() and expose it as a
# single tool in a real agent loop -- the pattern behind any "let the model
# run code" tool, hardened. The command below is kern's own documented
# pattern for untrusted/AI-generated code: a container (step 3's
# isolation) + --security-profile untrusted (step 4's seccomp allowlist) +
# a memory cap (step 2's resource limiting), together in one invocation.
