...
# Step 7: log in, then push (Docker Hub is the default registry if unqualified)
# docker login
# docker push myuser/docker_demo:1.0

# Step 7: pull a specific tag, or run it straight from the registry
docker pull alpine:3.20
docker run --rm myuser/docker_demo:1.0 2>/dev/null || echo "(image not actually pushed in this demo)"

# Step 7: a local registry for testing without touching Docker Hub
