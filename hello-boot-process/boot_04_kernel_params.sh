#!/usr/bin/env bash
# Run: sh boot_04_kernel_params.sh
# What this step adds: kernel boot parameters and where they come from.
# Step 4: kernel parameters

lsinitrd 2>/dev/null | head -3

# Step 4: the exact parameters the running kernel was booted with
cat /proc/cmdline

# Step 4: these come from the `linux` line GRUB passes, e.g.:
#   linux /vmlinuz-... root=UUID=... ro quiet splash

# Step 4: common parameters worth recognizing
...
# Step 4: some params are also readable live per-subsystem under sysctl
