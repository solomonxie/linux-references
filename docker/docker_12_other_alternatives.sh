#!/usr/bin/env sh
# Run: sh docker_12_other_alternatives.sh (reference notes, not meant to fully execute)
# What this step adds: advanced — the rest of the container tooling landscape.
# Step 12: other docker alternatives

podman run --rm alpine echo "via podman"

# Step 12: buildah — builds OCI images without a daemon, often paired with
# podman for running them; more scriptable/lower-level than `docker build`
# buildah bud -t myimage .

# Step 12: nerdctl — a docker-compatible CLI for containerd (the runtime
# docker itself uses under the hood), useful when only containerd is present
# nerdctl run --rm alpine echo hi

# Step 12: containerd / runc / crun — the actual low-level runtimes underneath
...
# Step 12: Desktop-integration alternatives to Docker Desktop, mostly for
...
# Step 12: rule of thumb — docker for the broadest ecosystem/tooling support,
